...
class Order(models.Model):
    ...
    'billing_street_address1', 'billing_town',
    'billing_county', 'billing_postcode', 'billing_country',
    'shipping_street_address1', 'shipping_town',
    'shipping_county', 'shipping_postcode', 'postage_class'

    Fields which are not editable are:
    'order_num', 'created_on', 'shipping_country', 'basket_contents',
      'parcel_size', 'order_subtotal',
    'order_discount', 'postage_cost', 'grand_total',
      'amount_payable', 'stripe_pid'

    Other fields:
    'billing_street_address2', 'shipping_street_address2',
    'is_gift', 'gift_message', 'use_voucher',
    ...
    **Model methods**
    ...
    order_num = models.CharField(
        max_length=8, null=False, editable=False, unique=True)
    ...
    billing_street_address1 = models.CharField(
        ...
        help_text="Orders can only be shipped to UK addresses")
    ...
    billing_town = models.CharField(
        max_length=50, null=False, blank=False, verbose_name="Town or City")
    billing_county = models.CharField(
        max_length=40, null=False, blank=False, verbose_name='County')
    billing_postcode = models.CharField(
        max_length=9, null=False, blank=False, verbose_name='Postcode')
    billing_country = models.CharField(
        max_length=20, default="GB", null=False, verbose_name='Country')
    ...
    amount_payable = models.DecimalField(
        max_digits=5, decimal_places=2, blank=False, default=0)
    ...
    def update_order(self):
        ...
        Function called by `signals.update_on_save()` and
        `signals.update_on_delete`
        """
        ball_count = self.lineitems.aggregate(
            Sum('quantity'))['quantity__sum'] or 0
        self.order_subtotal = self.lineitems.aggregate(
            Sum('linetotal'))['linetotal__sum'] or 0

        bulk_buy = Announcements.objects.filter(active=True)[0]

        yarn_weight = self.lineitems.aggregate(Sum('lineweight'))[
            'lineweight__sum'] or 0
        ...
        if ball_count < small_ball_limit and yarn_weight < small_weight_limit:
            self.parcel_size = 0
            self.postage_cost = Postage.objects.filter(Q(parcel_size=self.parcel_size) & Q(
                postage_class=self.postage_class))[0].postage_cost
        else:
            self.parcel_size = 1
            self.postage_cost = Postage.objects.filter(Q(parcel_size=self.parcel_size) & Q(
                postage_class=self.postage_class))[0].postage_cost

        if bulk_buy.bulk_buy:
            if ball_count > bulk_buy.upper_ball_num:
                self.order_discount = self.order_subtotal * \
                    Decimal((bulk_buy.upper_discount) / 100)
            elif ball_count < bulk_buy.lower_ball_num:
                self.order_discount = 0
            else:
                self.order_discount = self.order_subtotal * \
                    Decimal((bulk_buy.lower_discount) / 100)
        else:
            if ball_count > bulk_buy.upper_ball_num:
                self.order_discount = Postage.objects.filter(
                    Q(parcel_size=self.parcel_size) & Q(
                        postage_class=0))[0].postage_cost

        self.grand_total = self.order_subtotal - \
            self.order_discount + self.postage_cost
        if self.use_voucher:
    ...
    def save(self, *args, **kwargs):
        """
        Adds `order_num` to an instance of the :model:`Order` on save
...
class YarnOrderLineitem(models.Model):
    """
    Related to :model:`Order`, :model:`product.Colour_var` and
      :model:`core.SaleSettings`

    Fields are: 'order', 'yarn', 'quantity', 'current_price',
      'linetotal', 'lineweight'
    ...
    **Model methods**
    ...
    yarn = models.ForeignKey(Colour_var, null=True, blank=False,
                             on_delete=models.SET_NULL, related_name='yarn')
    quantity = models.IntegerField(null=False, blank=False)
    current_price = models.DecimalField(
        max_digits=5, decimal_places=2, blank=False, editable=False, default=0)
    ...
    def save(self):
        ...
        if self.yarn.product_id.on_promotion:
            self.current_price = Decimal(
                self.yarn.product_id.price * (100 - sale_discount) / 100)
        else:
...
class ReviewYarns(models.Model):
    ...
    Fields include are: 'order', 'yarn', 'rating', 'comment',
      'updated_on', 'approved'
    """
    ...
    def __str__(self):
        return f'{
            self.yarn.product_id.name} {
            self.yarn.colour_cat_id.colour_name}\
        left on {
            self.updated_on.strftime("%d-%b-%y")}'
