...
class Order(models.Model):
    ...
    **Model methods**
    ...
    billing_street_address1 = models.CharField(
        max_length=80, null=False, blank=False, verbose_name='Street Address 1', help_text="Orders can only be shipped to UK addresses")
        ...
        if bulk_buy.bulk_buy == True:
            ...
            if ball_count > bulk_buy.upper_ball_num:
                self.order_discount = Postage.objects.filter(
                    Q(parcel_size=self.parcel_size) & Q(postage_class=0))[0].postage_cost
...
class YarnOrderLineitem(models.Model):
    ...
    **Model methods**
    ...
    linetotal = models.DecimalField(
        max_digits=5, decimal_places=2, blank=False, null=False, editable=False)
...
class ReviewYarns(models.Model):
    ...
    yarn = models.ForeignKey(Colour_var, null=False, blank=False,
                             on_delete=models.CASCADE, related_name='line_item')
...
class Refund(models.Model):
    ...
    order = models.OneToOneField(
        Order, on_delete=models.CASCADE, blank=False, null=False, related_name='refunded_order')
    created_on = models.DateTimeField(auto_now_add=True)
...
class Shipped(models.Model):
    ...
    """
    order = models.OneToOneField(
        Order, on_delete=models.CASCADE, blank=False, null=False, related_name='shipped')
