...
class UserProfile(models.Model):
    ...
    default_phone = models.BigIntegerField(
        null=True, blank=True, verbose_name="Phone")
    ...
    default_town = models.CharField(
        max_length=50, null=True, blank=True, verbose_name='Town or City')
    default_county = models.CharField(
        max_length=40, null=True, blank=True, verbose_name='County')
...
class Postage(models.Model):
    ...
    postage_cost = models.DecimalField(
        max_digits=5, decimal_places=2, blank=False, verbose_name='cost in £')
    postage_class = models.IntegerField(choices=PCLASS, default=0)
    parcel_size = models.IntegerField(choices=PSIZE, default=0)
    max_weight = models.IntegerField(
        default=2, verbose_name="max weight in kg")
    max_no_balls = models.IntegerField(
        verbose_name="max number of balls for parcel size", default=50)
    updated_on = models.DateTimeField(auto_now=True)

    class Meta:
        ordering = ['postage_class']
...
class Announcements(models.Model):
    ...
    lower_ball_num = models.IntegerField(
        blank=True, verbose_name="number of balls in small bulk buy")
    lower_discount = models.IntegerField(
        verbose_name="percentage discount for small bulk buy", blank=True)
    upper_ball_num = models.IntegerField(
        blank=True, verbose_name="number of balls for large bulk buy")
    upper_discount = models.IntegerField(
        verbose_name="percentage discount for large bulk buy", blank=True)
...
class SaleSettings(models.Model):
    ...
    sale_percent = models.IntegerField(
        default=0, verbose_name="percentage discount for sale items")
    active = models.BooleanField(default=False)
    updated_on = models.DateTimeField(auto_now=True)
...
class ShopContactInfo(models.Model):
    ...
    shop_street_address1 = models.CharField(
        max_length=80, blank=True, verbose_name="Street Address 1")
    shop_street_address2 = models.CharField(
        max_length=80, blank=True, verbose_name="Street Address 2")
    shop_town = models.CharField(
        max_length=50, blank=True, verbose_name="Town")
    shop_county = models.CharField(
        max_length=40, blank=True, verbose_name="County")
    shop_country = models.CharField(default="GB", verbose_name="Country")
    shop_postcode = models.CharField(
        max_length=9, blank=True, verbose_name='Postcode')
    updated_on = models.DateTimeField(auto_now=True)
