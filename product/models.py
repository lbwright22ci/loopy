...
class Colour_cat(models.Model):
    ...
    shade_type_id = models.ForeignKey(
        'Shade_Type', null=True, blank=True, on_delete=models.SET_NULL, related_name="shade_type")
...
class Product(models.Model):
    ...
    brand_id = models.ForeignKey('Brand', null=True, blank=True,
                                 on_delete=models.CASCADE, related_name="brand", verbose_name='brand')
    thickness_id = models.ForeignKey('Thickness', null=True, blank=True,
                                     on_delete=models.SET_NULL, related_name="thickness", verbose_name='thickness')
    name = models.CharField(max_length=200, null=False)
    summary = models.CharField(max_length=500, null=True, blank=True)
    price = models.DecimalField(
        max_digits=6, decimal_places=2, blank=False, verbose_name="Price in £")
    ...
    skein_weight = models.IntegerField(
        blank=False, null=False, verbose_name='Weight of single ball in g')
    skein_length = models.IntegerField(
        blank=True, null=True, verbose_name="Length of yarn in a single ball in m")
    needle_size = models.DecimalField(max_digits=4, decimal_places=1, blank=True,
                                      null=True, verbose_name="recommended knitting needle size in mm")
    knitting_tension = models.CharField(
        max_length=300, blank=True, null=True, verbose_name="st and row count to produce 10 x 10 cm square")
    machine_wash = models.BooleanField(default=False)
    wash_temp = models.IntegerField(
        blank=True, null=True, verbose_name="Washing Temperature in deg C")
...
class Colour_var(models.Model):
    ...
    product_id = models.ForeignKey('Product', null=False, blank=False,
                                   on_delete=models.CASCADE, verbose_name='Yarn', related_name='product')
    colour_cat_id = models.ForeignKey('Colour_cat', null=True, blank=True,
                                      on_delete=models.SET_NULL, verbose_name='shade', related_name='colour')
