...
class Thickness(models.Model):
    ...
    alt_names = models.CharField(
        max_length=100, blank=True, verbose_name="US terminology")
...
class Shade_Type(models.Model):
    """ Stores instances of shade type so that specific
    colours can be grouped into generic terms
...
class Colour_cat(models.Model):
    """ Stores instances of specific shades of yarn.
    Each instance is related to :model:`shade_type`
...
class Product(models.Model):
    """ Stores instances of yarn products. Foreign keys
    are :model:`Brand` and :model:`Thickness`

    Fields include 'brand_id', 'thinkness_id', 'name', 'summary',
      'price', 'fibre',
    'skein_weight', 'skein_length', 'needle_size' , 'knitting_tension',
      'machine_wash'
    'wash_temp', 'tumble_dry', 'wool_cycle', 'slug', 'sku',
    'visible', 'on_promotion'
    'image
    """
    brand_id = models.ForeignKey('Brand', null=True, blank=True,
                                 on_delete=models.CASCADE,
                                 related_name="brand", verbose_name='brand')
    thickness_id = models.ForeignKey('Thickness', null=True, blank=True,
                                     on_delete=models.SET_NULL,
                                     related_name="thickness",
    ...
    skein_weight = models.IntegerField(
        blank=False, null=False,
        verbose_name='Weight of single ball in g')
    skein_length = models.IntegerField(
        blank=True, null=True,
        verbose_name="Length of yarn in a single ball in m")
    needle_size = models.DecimalField(max_digits=4, decimal_places=1,
                                      blank=True,
                                      null=True,
                                      verbose_name="recommended \
                                        knitting needle size in mm")
    knitting_tension = models.CharField(
        max_length=300, blank=True, null=True,
        verbose_name="st and row count to produce 10 x 10 cm square")
    machine_wash = models.BooleanField(default=False)
    wash_temp = models.IntegerField(
        blank=True, null=True,
        verbose_name="Washing Temperature in deg C")
    ...
    def save(self):
        ...
        if not self.natural_fibres:
            if self.fibre.__contains__('crylic') | (
                    self.fibre.__contains__('iscose')) | (
                    self.fibre.__contains__('oly')) | (
                    self.fibre.__contains__('ylon')) | (
                        self.fibre.__contains__('henille')):
...
class Colour_var(models.Model):
    """" This model stores instances of each specific product
    - the yarn type (:model:`Product`) and the
    specific colour option (:model:`Colour`)

    Fields include 'product_id', 'colour_cat_id', 'shade_code',
      'dye_lot', 'low_stock', 'in_stock'
    """
    product_id = models.ForeignKey('Product', null=False, blank=False,
                                   on_delete=models.CASCADE,
                                   verbose_name='Yarn', related_name='product')
    colour_cat_id = models.ForeignKey(
        'Colour_cat',
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
